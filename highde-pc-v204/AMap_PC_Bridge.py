...
class App(tk.Tk):
    def __init__(self):
        ...
        self.df=None;self.path='';self.cols={};self.q=queue.Queue();self.db=StateDB();self.server=BridgeServer(self.q);self.phone=False;self.phone_acc=False;self.running=False;self.paused=False;self.idx=0;self.current_task='';self.task_deadline=0
        self.build();self.server.start();self.status.config(text=f'连接服务已自动启动｜电脑IP {local_ip()}:{PORT}｜请使用安卓真机');self.after(120,self.tick)
    def build(self):
        top=tk.Frame(self,bg='#198754',height=72);top.pack(fill='x')
        tk.Label(top,text='高德App酒店信息补全器 · 真机稳定版 2.0.5',font=('Microsoft YaHei',22,'bold'),fg='white',bg='#198754').pack(side='left',padx=22,pady=16)
        ...
        cols=('序号','酒店名称','POI_ID','开业时间','装修时间','客房数','App电话','名称校验','状态');self.tree=ttk.Treeview(self,columns=cols,show='headings')
    ...
    def import_xls(self):
        ...
        self.path=p;cs=list(self.df.columns);self.cols={'poi':guess_col(cs,['高德POI_ID','POI_ID','poi_id']),'name':guess_col(cs,['酒店名称','名称','name']),'lon':guess_col(cs,['经度','longitude','lon']),'lat':guess_col(cs,['纬度','latitude','lat'])}
    ...
    def start_job(self):
        ...
        if not self.phone_acc:return messagebox.showwarning('手机无障碍未开启','请打开手机“高德酒店采集助手”→开启无障碍读取权限→再连接电脑')
        self.running=True;self.paused=False;self.idx=0;self.write('开始补全；手机无障碍状态正常');self.next_task()
    def next_task(self):
        if not self.running or self.paused:return
        while self.idx<len(self.df):
            i=self.idx;self.idx+=1;r=self.df.iloc[i];poi=str(r.get(self.cols['poi'],'') or '').strip();name=str(r.get(self.cols['name'],'') or '').strip();old=self.db.get(poi)
            if old and old.get('status')=='success':continue
            task_id=str(uuid.uuid4())[:10];self.current_task=task_id;self.task_deadline=time.time()+32
    ...
    def handle_msg(self,j):
        t=j.get('type')
        if t=='hello':
            self.phone=True;self.phone_acc=bool(j.get('accessibility'));self.status.config(text=f'手机已连接：{j.get("device","")} | 版本 {j.get("version","")} | 无障碍：{"已开启" if self.phone_acc else "未开启"}');self.write('手机连接成功；无障碍='+('已开启' if self.phone_acc else '未开启'))
        elif t=='task_opened':self.write('手机已收到任务；高德调起'+('成功' if j.get('ok') else '失败'))
        elif t=='status':self.phone_acc=bool(j.get('accessibility'));self.write('手机状态：无障碍='+('在线' if self.phone_acc else '离线'))
        elif t=='result':
            self.db.save(j);poi=j.get('poi_id','');st=j.get('status','')
            for iid in self.tree.get_children():
                vals=list(self.tree.item(iid,'values'))
                if len(vals)>2 and str(vals[2])==poi:
                    vals[3]=j.get('open_time','');vals[4]=j.get('renovate_time','');vals[5]=j.get('rooms','');vals[6]=j.get('phone','');vals[7]=j.get('name_check','');vals[8]=st;self.tree.item(iid,values=vals);self.tree.see(iid);break
            self.write(f'回传: {j.get("name")} | 开业={j.get("open_time")} | 状态={st} | {j.get("evidence","")}')
            if st=='accessibility_off':
                self.phone_acc=False;self.running=False;self.current_task='';messagebox.showwarning('手机无障碍未开启','手机端没有开启“高德酒店采集助手”无障碍服务。\n请先开启后重新连接，不会继续跳过后面的酒店。');return
    ...
    def tick(self):
        ...
        if self.running and self.current_task and time.time()>self.task_deadline:
            self.write('当前酒店超时：手机没有返回结果。请看上面的“手机调试”行判断是高德页面还是无障碍读取问题。');self.current_task='';self.running=False;messagebox.showwarning('任务暂停','第一家超时，已自动暂停全量任务，避免连续跳过。请根据运行日志定位后再继续。')
