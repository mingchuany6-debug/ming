...
public class AmapAccessibilityService extends AccessibilityService {
    ...
    private long lastRun=0; private int scrollCount=0; private boolean clickedInfo=false;
    ...
    @Override public void onAccessibilityEvent(AccessibilityEvent event){
        ...
        long now=System.currentTimeMillis(); if(now-lastRun<650)return; lastRun=now;
        h.removeCallbacks(scanRunnable); h.postDelayed(scanRunnable,550);
    }
    ...
    private void scan(){
        if(!TaskState.active)return;
        AccessibilityNodeInfo root=getRootInActiveWindow(); if(root==null){h.postDelayed(scanRunnable,700);return;}
        ArrayList<String> texts=new ArrayList<>(); collect(root,texts,0);
        String raw=String.join(" | ",texts); if(raw.length()>16000) raw=raw.substring(0,16000);

        if(!clickedInfo && clickAny(root,new String[]{"酒店信息","酒店详情","设施服务","详情","更多信息","更多"})){
            clickedInfo=true; h.postDelayed(scanRunnable,850); return;
        }

        Result r=parse(raw,TaskState.hotelName);
        long elapsed=System.currentTimeMillis()-TaskState.startedAt;
        if(!r.open.isEmpty() && (elapsed>1800 || !r.rooms.isEmpty() || !r.phone.isEmpty())){
        ...
        if(elapsed>18000 || scrollCount>=10){ finish(r,r.open.isEmpty()?"open_time_not_found":"partial",raw); }
    ...
    private void finish(Result r,String status,String raw){
        ...
        TaskState.clear(); scrollCount=0; clickedInfo=false;
    }

    private void collect(AccessibilityNodeInfo n,List<String> out,int d){
        if(n==null||d>40||out.size()>1200)return;
    ...
    private boolean clickAny(AccessibilityNodeInfo n,String[] keys){
        ...
        if(t!=null){
            ...
            for(String k:keys) if(s.equals(k)||s.contains(k)){
                ...
                for(int i=0;i<5&&c!=null;i++,c=c.getParent()) if(c.isClickable()&&c.performAction(AccessibilityNodeInfo.ACTION_CLICK))return true;
            }
        }
